...
export default async function ProductsPage({
    ...
    // Festival badges
    (sp.festival ? (Array.isArray(sp.festival) ? sp.festival : [sp.festival]) : []).forEach((f) =>
        activeBadges.push(String(f).split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '))
    );

    // Category badges
    (sp.category ? (Array.isArray(sp.category) ? sp.category : [sp.category]) : []).forEach((c) =>
        activeBadges.push(String(c).split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '))
    );

    // Pack Type badges
    (sp.packType ? (Array.isArray(sp.packType) ? sp.packType : [sp.packType]) : []).forEach((pt) =>
        activeBadges.push(String(pt).charAt(0).toUpperCase() + String(pt).slice(1))
    );
                            ...
                            {products.map((p) => {
                                ...
                                return (
                                    <Card
                                        ...
                                        imageSrc={p.imageUrl ?? "/trading-cards/pack-1.png"}
                                        price={price}
                                        href={`/products/${p.id}`}
                                    />
                                );
